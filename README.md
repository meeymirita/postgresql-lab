...
- **Сессия 1** — окружение и данные: Postgres в Docker с инструментами наблюдения, `psql` как рабочее место, схема кофейни, миллион заказов за минуту, SQL-инструментарий (GROUP BY/HAVING, EXISTS, CTE, оконные функции, FILTER, LATERAL)
...
- **Сессия 3** — три алгоритма JOIN и `work_mem`, GIN (триграммы, массивы, jsonb, полнотекстовый поиск), BRIN, расширенная статистика, N+1 через `\gexec` и `pg_stat_statements`, пагинация, охота на медленные запросы
